...
- Prefer `pnpm test -- <path-or-filter> [vitest args...]` for local debugging. It runs the repo wrapper (`scripts/test-parallel.mjs`), which picks the right config/pool defaults and can route targeted files to the right suite automatically.
...
- `pnpm test:e2e`: Runs gateway end-to-end smoke tests (multi-instance WS/HTTP/node pairing). Uses process `forks`, defaults to `1` worker locally, and keeps CI workers conservative; tune with `OPENCLAW_E2E_WORKERS=<n>` and set `OPENCLAW_E2E_VERBOSE=1` for verbose logs.
- `pnpm test:live`: Runs provider live tests. The script already sets `OPENCLAW_LIVE_TEST=1` and `CLAWDBOT_LIVE_TEST=1`; individual live suites may still require allowlist env vars or provider-specific `*_LIVE_TEST=1` flags.
